...
// location using a std::list by copying it into a vector, sorting and 
...
int main(int argc, char *argv[]){
	...
	vector<location> v{begin(locs),end(locs)};	// makes a vector with the locations in locs
	sort(v.begin(),v.end());			// sorts the vector
	locs.clear();					// clear original locs list
	copy(v.begin(),v.end(),back_inserter(locs));	// populates locs list with the elements from the sorted vector
	for(iterator = locs.begin(); iterator != locs.end(); iterator++){
