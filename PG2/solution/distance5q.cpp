...
int partition(vector<poi> &v, const Less &less, const int p, const int q) {
  poi z = v[p];
  int i = p;
...
template <typename Less>
void quicksort(vector<poi> &v, const Less &less, const int p, const int q) {
  int r;
  if (p < q) {
    r = partition(v, less, p, q);
    quicksort(v, less, p, r);
    quicksort(v, less, r + 1, q);
  }
}
...
int main (int argc, char *argv[]) {
  ...
  quicksort(container, dist_comp(*me), 0, container.size());
